...
int main()
{
  ...
  int s;//characters  before @
  int e;//characters character after @
  bool hasDot;//check for dot
  string anEmail;//emails
  ...
  while (fin.good())
  {
    ...
    // Find an @ sign and if it has it get s and e
    for (i = 0; i < readLines.length(); i++)
    {
      ...
      if(readLines[i] == '@')
      {
        ...
        // Find how many characters are valid
        while (isValidEmailCharacter (readLines[s]))
        {
          s--;
        }//while

        while (isValidEmailCharacter (readLines[e]))
        {
          e++;
          if (readLines[e] == '.')
          {
        ...
        anEmail = readLines.substr(s, e-s);
      }//if  
    }//for
